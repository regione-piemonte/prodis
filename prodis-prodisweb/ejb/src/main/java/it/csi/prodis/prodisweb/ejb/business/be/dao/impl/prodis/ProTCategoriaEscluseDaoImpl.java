...
public class ProTCategoriaEscluseDaoImpl extends BaseEntityDaoImpl<Long, ProTCategoriaEscluse> implements ProTCategoriaEscluseDao {
	...
	public List<DecodificaGenerica> findByFilter(DecodificaGenerica filtro) {
		...
		sql.append(" SELECT DISTINCT  ID_T_CATEGORIA_ESCLUSE id_dec, COD_CATEGORIA_ESCLUSE cod_dec, DES_CATEGORIA_ESCLUSE des_dec, AMBITO_CATEGORIA ambito "); 
	    
	    sql.append(" FROM  PRO_T_CATEGORIA_ESCLUSE  WHERE 1=1 ");
	    if (filtro.getIdDecodifica() != null ) {
	    	 params.put("idDecod", filtro.getIdDecodifica().toString());
	    	 sql.append(" AND ID_T_CATEGORIA_ESCLUSE = :idDecod");
	    } else {
	    	if (filtro.getCodDecodifica() != null) {
	    		params.put("codDecod",  "%" + filtro.getCodDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(COD_CATEGORIA_ESCLUSE) like :codDecod");
	    	}  
	    	if (filtro.getDsDecodifica() != null) {
	    		params.put("dsDecod", "%" + filtro.getDsDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(DES_CATEGORIA_ESCLUSE) like :dsDecod");
	    	}  
	    	if (filtro.getAmbitoDecodifica() != null) {
	    		params.put("ambito", "%" + filtro.getAmbitoDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(AMBITO_CATEGORIA) like :ambito");
	    	}   
	    } 
