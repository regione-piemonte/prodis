...
public class ProTCausaSospensioneDaoImpl extends BaseEntityDaoImpl<Long, ProTCausaSospensione> implements ProTCausaSospensioneDao {
	...
	public List<DecodificaGenerica> findByFilter(DecodificaGenerica filtro) {
		...
		sql.append(" SELECT DISTINCT  ID_T_CAUSA_SOSPENSIONE id_dec, COD_CAUSA_SOSPENSIONE cod_dec, DES_CAUSA_SOSPENSIONE des_dec "); 
	    sql.append(" FROM  PRO_T_CAUSA_SOSPENSIONE  WHERE 1=1 ");
	    if (filtro.getIdDecodifica() != null ) {
	    	 params.put("idDecod", filtro.getIdDecodifica().toString());
	    	 sql.append(" AND ID_T_CAUSA_SOSPENSIONE = :idDecod");
	    } else {
	    	if (filtro.getCodDecodifica() != null) {
	    		params.put("codDecod",  "%" + filtro.getCodDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(COD_CAUSA_SOSPENSIONE) like :codDecod");
	    	}  
	    	if (filtro.getDsDecodifica() != null) {
	    		params.put("dsDecod", "%" + filtro.getDsDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(DES_CAUSA_SOSPENSIONE) like :dsDecod");
	    	}  
	    } 
