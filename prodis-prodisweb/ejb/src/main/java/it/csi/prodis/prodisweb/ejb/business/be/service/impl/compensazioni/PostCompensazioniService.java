...
public class PostCompensazioniService extends BaseCompensazioniService<PostCompensazioniRequest, PostCompensazioniResponse> {
	/**
	 * Constructor
	 * 
	 ...
	 */
	public PostCompensazioniService(ConfigurationHelper configurationHelper, CompensazioniDad compensazioniDad) {
	...
	protected void execute() {
		...
		if (apiErrors == null || apiErrors.size() == 0) {
			if (compensazioni.getId()==null) {
				compensazioni = compensazioniDad.insertCompensazioni(compensazioni, compensazioni.getIdProspettoProv());
			} else {
