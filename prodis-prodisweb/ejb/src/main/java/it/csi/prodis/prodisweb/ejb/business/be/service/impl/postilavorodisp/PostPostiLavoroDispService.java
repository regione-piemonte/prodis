...
public class PostPostiLavoroDispService extends BasePostiLavoroDispService<PostPostiLavoroDispRequest, PostPostiLavoroDispResponse> {
	/**
	 * Constructor
	 * 
	 ...
	 */
	public PostPostiLavoroDispService(ConfigurationHelper configurationHelper, PostiLavoroDispDad postiLavoroDispDad) {
	...
	protected void execute() {
		...
		if (apiErrors == null || apiErrors.size() == 0) {
			if (postiLavoroDisp.getId()==null) {
				postiLavoroDisp = postiLavoroDispDad.insertPostiLavoroDisp(postiLavoroDisp, postiLavoroDisp.getIdProspettoProv());
			} else {
