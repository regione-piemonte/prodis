...
public class ProTContrattiDaoImpl extends BaseEntityDaoImpl<Long, ProTContratti> implements ProTContrattiDao {
	...
	public List<DecodificaGenerica> findByFilter(DecodificaGenerica filtro) {
		...
		sql.append(" SELECT  DISTINCT  ID_T_CONTRATTO id_dec, COD_TIPO_CONTRATTO_MIN cod_dec, DS_CONTRATTO des_dec, FLG_FORMA "); 
	    sql.append(" FROM  PRO_T_contratti  WHERE 1=1 ");
	    if (filtro.getIdDecodifica() != null ) {
	    	 params.put("idDecod", filtro.getIdDecodifica().toString());
	    	 sql.append(" AND ID_T_CONTRATTO = :idDecod");
	    } else {
	    	if (filtro.getCodDecodifica() != null) {
	    		params.put("codDecod",  "%" + filtro.getCodDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(COD_TIPO_CONTRATTO_MIN) like :codDecod");
	    	}  
	    	if (filtro.getDsDecodifica() != null) {
	    		params.put("dsDecod", "%" + filtro.getDsDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(DS_CONTRATTO) like :dsDecod");
	    	}   
	    } 
	...
	@Override
	public Optional<ProTContratti> findByCodice(String codiceTipologiaContrattuale) {
		...
		JpaQueryHelper.andStringLike(jpql, params, "t.codTipoContrattoMin", "codTipoContrattoMin", codiceTipologiaContrattuale);
