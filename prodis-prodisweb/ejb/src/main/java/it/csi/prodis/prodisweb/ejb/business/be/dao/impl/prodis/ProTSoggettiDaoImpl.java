...
/**
 * Data Access Object implementor for the entity ProTSoggetti
 */
@ApplicationScoped
public class ProTSoggettiDaoImpl extends BaseEntityDaoImpl<Long, ProTSoggetti> implements ProTSoggettiDao {

	@Override
	public List<ProTSoggetti> findByField(String field) {
		Map<String, Object> params = new HashMap<String, Object>();
		StringBuilder jpql = new StringBuilder();
		jpql.append(" FROM ProTSoggetti t ");
		jpql.append(" WHERE 1=1 ");

		JpaQueryHelper.andFieldEquals(jpql, params, "t.field", "field", field);

		jpql.append(" ORDER BY t.idTSoggetti ");

		TypedQuery<ProTSoggetti> query = composeTypedQuery(jpql, params);
	...
	public List<DecodificaGenerica> findByFilter(DecodificaGenerica filtro) {
		...
		sql.append(" SELECT DISTINCT  ID_T_SOGGETTI id_dec , COD_SOGGETTI cod_dec, DESC_SOGGETTI des_dec ");
        sql.append(" FROM  PRO_T_SOGGETTI  WHERE 1=1 ");
		if (filtro.getIdDecodifica() != null) {
	    	 params.put("idDecod", filtro.getIdDecodifica().toString());
	    	 sql.append(" AND ID_T_SOGGETTI = :idDecod");
		} else {
			if (filtro.getCodDecodifica() != null) {
	    		params.put("codDecod",  "%" + filtro.getCodDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(COD_SOGGETTI) like :codDecod");
			}
			if (filtro.getDsDecodifica() != null) {
	    		params.put("dsDecod", "%" + filtro.getDsDecodifica().toUpperCase() + "%");
	    		sql.append(" AND upper(DESC_SOGGETTI) like :dsDecod");
			}
		}
	...
	@Override
	public ProTSoggetti findBycodSoggetti(String codSoggetti) {

		Map<String, Object> params = new HashMap<>();

		StringBuilder jpql = new StringBuilder()
			.append("FROM ProTSoggetti s ")
			.append(" WHERE 1 = 1 ");

		JpaQueryHelper.andFieldEquals(jpql, params, "s.codSoggetti", "codSoggetti", codSoggetti);


		TypedQuery<ProTSoggetti> query = composeTypedQuery(jpql, params);
		List<ProTSoggetti> results = query.getResultList();
