 ...
 * =========================LICENSE_END==================================
 */
package it.csi.prodis.prodisweb.lib.utils;
...
// confronto particolare fra due date: oltre all'anno, mese, giorno, anche ore, minuti e secondi
      ...
      final String replacement)
  {
    ...
    if ( (idxOld = input.indexOf(exp, startIdx)) >= 0)
    {
      ...
      // conterr� l'indice dell'ultimo char della prima occorrenza
...
//    java.text.ParsePosition pos = new ParsePosition(0);
  ...
  public static java.util.Date convertiStringaConOreMinutiSecondiInData(String
    ...
    //se la parte contenente ore minuti secondi ha pi� di 5 caratteri vuol dire che ha anche i secondi
...
//  }
   ...
   * La stringa di ritorno sar� costituita da una lettera dell'alfabeto ottenuta dai controlli
   * che devono essere effettuati sul codice fiscale che viene passato.
   * Il calcolo � realizzato secondo una mappa associativa propria dell'algoritmo di calcolo.
   ...
   * a seconda del numero trovato si avr� una lettera dell'alfabeto corrispondente
   * <li> GIORNO:
   * nel caso in cui il sesso � 'M' � dato dal primo e secondo carattere della data di nascita
   * nel caso in cui il sesso � 'F' � dato dal numero prima ricavato + 40
   * </ul>
   ...
   * @return Stringa in formato HH:MM (Compresi di 0). Restiutisce null se l'input � null
   */
   ...
   * @return minuti dalla mezzanotte. Restiutisce null se l'input � null
   */
  ...
  /**
   * Restituisce true se l'ora � minore o uguale a max e maggiore o uguale a min
   * @param ora
   * @param min
   * @param max
   * @return
  ...
  /*
   ...
   mentre i periodi superiori a giorni quindici si computano come un mese intero.
    ...
    quanto il periodo � superiore a 15 giorni
  ...
  /*
   ...
   quando si � nell'ambito SILP / STATI OCCUPAZIONALI (quindi anche provvedimenti..)
...
//  }
  ...
  /**
   *  Restituisce un intero contenente l'et� calcolata ad una
   *   certa data di riferimento (per l'et� al momento corrente
   *   si passa la sysdate)
   *  Non lancia eccezioni: se il calcolo fallisce per qualche motivo
   * l'et� restituita � un insensato  -1
   * @param dataNascita
...
//     * il synchronized � necessario solo quando si crea l'istanza Singleton
//     * ATTENZIONE: � necessario controllare prima il NULL per evitare di creare
