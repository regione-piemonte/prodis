 ...
 * =========================LICENSE_END==================================
 */
package it.csi.prodis.prodisweb.protocollo.hepler;
...
import it.csi.iup.iupsv.cxfclient.DatiAnnulloProtocolloDTO;
...
import it.csi.prodis.prodisweb.lib.exception.IupIntegrationException;
...
public class AdapterIup {
	...
	private static final String USER = "PRODIS";
	private static final String COD_APPLICATIVO = "PRODIS";
	...
	public Properties staccaProtocollo(Properties p)  throws Exception {
		...
		try {
			...
			datiLoginProtocollo.setLoginProtocollo(USER);
			datiLoginProtocollo.setIdProvincia(REGIONE_PIEMONTE);
		...
		private IupsvSrv getService() throws MalformedURLException  {
			...
			if(srv == null) {
				...
				try {
					...
					URL wsdlURL = IupsvSrv.class.getClassLoader().getResource("iup.iupsv.wsdl").toURI().toURL(); // FIXME
