...
//	private static final Logger logger = Logger.getLogger(Constants.LOGGER);
	...
	private AAEPInterface2 getService() throws Exception {
		if (service == null) {
			// InputStream pdJaxb =
			// this.getClass().getResourceAsStream("jaxb.properties");
			// System.out.println("pdJaxb: " + pdJaxb);
	...
	public List<it.csi.prodis.prodisweb.profilazione.dto.ImpresaInfoc> cercaElencoAziende(String codiceFiscale)
			throws Exception {
		...
		try {
			...
			if (elencoPersonaCaricaInfoc != null && elencoPersonaCaricaInfoc.length > 0) {
				...
				for (ListaPersonaCaricaInfoc personaCaricaInfoc : elencoPersonaCaricaInfoc) {
					try {
						...
						Conversioni util = new Conversioni();
						GregorianCalendar dataCessaz = util.stringToDate(aziendaInfoc.getDataCessaz());
						GregorianCalendar dataDenunciaCessaz = util.stringToDate(aziendaInfoc.getDataDenunciaCessaz());
			...
			List<it.csi.prodis.prodisweb.profilazione.dto.ImpresaInfoc> result = new ArrayList<>();
			for (ImpresaInfoc key : elencoAziendeRis.keySet()) {
				result.add(converti(key));
			}
			return result;
		} finally {
			watcher.dumpElapsed(ProdisClassUtils.truncClassName(getClass()), "cercaElencoAziende()",
					"invocazione servizi AAEP " + ProdisClassUtils.truncClassName(getClass()) + ".cercaElencoAziende",
					"");
			watcher.stop();
		}
	}
	 ...
	 */
	private it.csi.prodis.prodisweb.profilazione.dto.ImpresaInfoc converti(ImpresaInfoc iWS) {
		it.csi.prodis.prodisweb.profilazione.dto.ImpresaInfoc i = new it.csi.prodis.prodisweb.profilazione.dto.ImpresaInfoc();
		...
		if (iWS.getListaSediInfoc() != null) {
			List<it.csi.prodis.prodisweb.profilazione.dto.ListaSediInfoc> listaSediInfoc = new ArrayList<>();
	 ...
	 * Converte i dati restituiti dal servizio di AAEP in DTO di prodis.
	 */
	private it.csi.prodis.prodisweb.profilazione.dto.ListaSediInfoc converti(ListaSediInfoc lWS) {
		it.csi.prodis.prodisweb.profilazione.dto.ListaSediInfoc l = new it.csi.prodis.prodisweb.profilazione.dto.ListaSediInfoc();
	...
	public AziendaAAEP cercaPerCodiceFiscale(String codiceFiscale) throws Exception {
		...
		try {
			...
			// TO DO :: Implementare la ricerca della descrizione dell'azienda tramite
			// chiamata all'opportuno metodo
			// Azienda azienda = getService().cercaPerCodiceFiscale(codiceFiscale, "",
			// "");
