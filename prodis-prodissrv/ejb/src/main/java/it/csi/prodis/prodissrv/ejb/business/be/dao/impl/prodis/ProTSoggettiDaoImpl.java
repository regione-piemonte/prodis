...
public class ProTSoggettiDaoImpl extends BaseEntityDaoImpl<Long, ProTSoggetti> implements ProTSoggettiDao {
	...
	public List<DecodificaGenerica> findByFilter(DecodificaGenerica filtro) {
		...
		sql.append(" SELECT ");
		sql.append(" DISTINCT ");
		sql.append("  ID_T_SOGGETTI id_dec ");
		sql.append(" , COD_SOGGETTI cod_dec  ");
		sql.append(" , DESC_SOGGETTI des_dec ");

		sql.append(" FROM ");
		sql.append("  PRO_T_SOGGETTI ");
		sql.append(" WHERE 1=1 ");
		if (filtro.getIdDecodifica() != null) {
			sql.append(" AND ID_T_SOGGETTI = ");
			sql.append(filtro.getIdDecodifica().toString());
		} else {
			if (filtro.getCodDecodifica() != null) {
				sql.append(" AND upper(COD_SOGGETTI) like '%");
				sql.append(filtro.getCodDecodifica().toUpperCase());
				sql.append("%'");
			}
			if (filtro.getDsDecodifica() != null) {
				sql.append(" AND upper(DESC_SOGGETTI) like '%");
