...
import { ApiError, CommonService, Prospetto, Ruolo } from 'src/app/modules/prodisapi';
import { Utils } from 'src/app/utils';
import { isArray, isNullOrUndefined } from 'util';
import { STATO_PROSPETTO_ANNULLATA, STATO_PROSPETTO_BOZZA, STATO_PROSPETTO_CANCELLATA, STATO_PROSPETTO_FIRMARE, STATO_PROSPETTO_IN_ANNULLAMENTO, STATO_PROSPETTO_IN_RETTIFICA, STATO_PROSPETTO_PRESENTATA, STATO_PROSPETTO_RETTIFICATA } from '../constants';
import { ErrorModalComponent } from '../modules/prodiscommon/components/error-modal/error-modal.component';
import { ProdisStorageService } from './storage/prodis-storage.service';
...
export class UtilitiesService {
  ...
  public getProspetto(): Prospetto {
    this.prodisStorageService.prospetto$.subscribe(async item => {
      this.ilProspetto = item;
    });
    return this.ilProspetto;
  }


  public isProspettoStatoBozza(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_BOZZA) {
        return true;
    }
    return false;
  }

  public isProspettoDaFirmare(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_FIRMARE) {
        return true;
    }
    return false;
  }

  public isProspettoPresentato(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_PRESENTATA) {
        return true;
    }
    return false;
  }

  public isProspettoInRettifica(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_IN_RETTIFICA) {
        return true;
    }
    return false;
  }

  public isProspettoRettificata(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_RETTIFICATA) {
        return true;
    }
    return false;
  }

  public isProspettoAnnullata(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_ANNULLATA) {
        return true;
    }
    return false;
  }

  public isProspettoInAnnullamento(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_IN_ANNULLAMENTO) {
        return true;
    }
    return false;
  }

  public isProspettoCancellata(ilProspetto: Prospetto): boolean {
    if (!isNullOrUndefined(ilProspetto) && ilProspetto.statoProspetto.descrizione === STATO_PROSPETTO_CANCELLATA) {
      return true;
    }
    return false;
  }
  ...
  async getRuoli(): Promise<Ruolo[]> {
    return this.commonService.getRuolo().pipe(
      map((result: Ruolo[]) => {
        return result;
      }
      ),
